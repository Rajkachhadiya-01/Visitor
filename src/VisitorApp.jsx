...
// const ResidentDashboard = ({ visitors, approvals, notifications, onLogout, onAddApproval, onApproveRequest, onApproveVisitor, onRejectVisitor }) => {
// const myVisitors = visitors.filter(v => v.flat === 'A-101');
// const myApprovals = approvals.filter(a => a.flat === 'A-101' && a.approved);
// const pendingRequests = approvals.filter(a => a.flat === 'A-101' && !a.approved);
// const pendingVisitors = visitors.filter(v => v.flat === 'A-101' && v.status === 'pending');
// const inside = myVisitors.filter(v => v.status === 'inside').length;
...
// <div>
// <h1 className="text-2xl font-bold">Resident Dashboard</h1>
// <p className="text-indigo-200">Flat A-101 - Amit Kumar</p>
// </div>
// <button onClick={onLogout} className="bg-white text-indigo-600 px-4 py-2 rounded-lg font-semibold">
// Logout
// </button>
...
// if (view === 'login') {
// return <LoginScreen onLogin={(role) => setView(role === 'resident' ? 'resident-dash' : 'security-dash')} />;
// }
